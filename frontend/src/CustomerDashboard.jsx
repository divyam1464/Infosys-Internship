...
//   }
  ...
  const fetchQuotations = async () => {
    try {
      ...
      const res = await fetch(`http://127.0.0.1:8000/api/quotations/customer/${userId}`);
      const data = await res.json();
      
      if (data.quotations) {
        // This perfectly matches your frontend table structure
        const mappedQuotes = data.quotations.map(q => ({
            quote_id: `Q-${q.id}`,
            origin: q.route_data.origin,
            destination: q.route_data.destination,
            cargo_type: q.route_data.cargo_type,
            containers: q.route_data.containers,
            route_name: q.route_data.route_name,
            route_score: q.route_data.route_score,
            transit_days: q.route_data.transit_days,
            total_cost_usd: q.total_price,
  ...
  const handleProceedToBooking = async () => {
    ...
    try {
      ...
      const totalCost = selectedRoute.pricing.total_cost_usd;
      const baseFreight = selectedRoute.pricing.breakdown.base_freight * formData.containers;
      const marginValue = totalCost * 0.15; 
      ...
      if (res.ok) {
        setNotification(`Quote saved! Sent to Admin for approval.`);
        setTimeout(() => setNotification(null), 4000);
        setActiveTab("quotations");
        fetchQuotations(); // This forces the list to update instantly
      }
    } catch (error) {
